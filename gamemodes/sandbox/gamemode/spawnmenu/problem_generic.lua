...
local bgClr = Color( 20, 28, 48, 200 )
function PANEL:Paint( w, h )
	...
	if ( self.Problem.lastOccurence ) then
		-- Son hata üzerinden zaman geçtikçe yavaşça sönümlenen hafif parlama
		local t = math.Clamp( ( SysTime() - self.Problem.lastOccurence ) / 2, 0, 1 )
		local r = math.floor( 20 + ( 35 - 20 ) * ( 1 - t ) )
		local g = math.floor( 28 + ( 80 - 28 ) * ( 1 - t ) )
		local b = math.floor( 48 + ( 90 - 48 ) * ( 1 - t ) )
		bgClrH = Color( r, g, b, self:GetAlpha() )
	end
...
local white = Color( 224, 236, 255, 255 )
local bg = Color( 15, 22, 40, 210 )
function PANEL:Paint( w, h )
